...
class Asteroid:
    __slots__ = ('x', 'y', 'vx', 'vy', 'size', 'mass', 'radius', 'timesteps_until_appearance', 'alive')

    def __init__(self, x: float = 0.0, y: float = 0.0, vx: float = 0.0, vy: float = 0.0, size: i64 = 0, mass: float = 0.0, radius: float = 0.0, timesteps_until_appearance: i64 = 0) -> None:
        self.x = x
        self.y = y
        self.vx = vx
        self.vy = vy
    ...
    def __str__(self) -> str:
        return f'Asteroid(position=({self.x}, {self.y}), velocity=({self.vx}, {self.vy}), size={self.size}, mass={self.mass}, radius={self.radius}, timesteps_until_appearance={self.timesteps_until_appearance})'
    ...
    def __hash__(self) -> i64:
        ...
        combined: float = self.x + 0.4266548291679171*self.y + 0.8164926348982552*self.vx + 0.8397584399461026*self.vy
    ...
    def int_hash(self) -> i64:
        return i64(1_000_000_000*(self.x + 0.4266548291679171*self.y + 0.8164926348982552*self.vx + 0.8397584399461026*self.vy))

    def copy(self) -> 'Asteroid':
        if not self.alive:
            raise Exception("Trying to copy unalive object")
        return Asteroid(
            x=self.x,
            y=self.y,
            vx=self.vx,
...
class Mine:
    __slots__ = ('x', 'y', 'mass', 'fuse_time', 'remaining_time', 'alive')

    def __init__(self, x: float = 0.0, y: float = 0.0, mass: float = 0.0, fuse_time: float = 0.0, remaining_time: float = 0.0) -> None:
        self.x = x
        self.y = y
        self.mass = mass
    ...
    def __str__(self) -> str:
        return f'Mine(position=({self.x}, {self.y}), mass={self.mass}, fuse_time={self.fuse_time}, remaining_time={self.remaining_time})'
...
class Bullet:
    __slots__ = ('x', 'y', 'vx', 'vy', 'heading', 'mass', 'tail_delta_x', 'tail_delta_y', 'alive')

    def __init__(self, x: float = 0.0, y: float = 0.0, vx: float = 0.0, vy: float = 0.0, heading: float = 0.0, mass: float = BULLET_MASS, tail_delta_x: float = 0.0, tail_delta_y: float = 0.0) -> None:
        self.x = x
        self.y = y
        self.vx = vx
        self.vy = vy
        self.heading = heading
        self.mass = mass
        self.tail_delta_x = tail_delta_x
        self.tail_delta_y = tail_delta_y
        self.alive: bool = True
        '''
        if tail_delta_x is not None:
            self.tail_delta_x = tail_delta_x
        else:
            self.tail_delta_x = -BULLET_LENGTH*cos(heading*DEG_TO_RAD)
        if tail_delta_y is not None:
            self.tail_delta_y = tail_delta_y
        else:
            self.tail_delta_y = -BULLET_LENGTH*cos(heading*DEG_TO_RAD)
        '''

    def __str__(self) -> str:
        return f'Bullet(position=({self.x}, {self.y}), velocity=({self.vx}, {self.vy}), heading={self.heading}, mass={self.mass}, tail_delta=({self.tail_delta_x}, {self.tail_delta_y}))'
    ...
    def copy(self) -> 'Bullet':
        ...
        return Bullet(
            ...
            heading=self.heading,
            mass=self.mass,
            tail_delta_x=self.tail_delta_x,
            tail_delta_y=self.tail_delta_y,
        )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (self.x == other.x and
                ...
                self.heading == other.heading and
                self.mass == other.mass and
                self.tail_delta_x == other.tail_delta_x and
                self.tail_delta_y == other.tail_delta_y)


class GameState:
    __slots__ = ('asteroids', 'ships', 'bullets', 'mines', 'map_size_x', 'map_size_y', 'time', 'delta_time', 'sim_frame', 'time_limit')

    def __init__(self, asteroids: list[Asteroid], ships: list[Ship], bullets: list[Bullet], mines: list[Mine], map_size_x: float = 0.0, map_size_y: float = 0.0, time: float = 0.0, delta_time: float = 0.0, sim_frame: i64 = 0, time_limit: float = 0.0) -> None:
    ...
    def __str__(self) -> str:
        return f'GameState(asteroids={self.asteroids}, ships={self.ships}, bullets={self.bullets}, mines={self.mines}, map_size=({self.map_size_x}, {self.map_size_y}), time={self.time}, delta_time={self.delta_time}, sim_frame={self.sim_frame}, time_limit={self.time_limit})'
    ...
    def copy(self) -> 'GameState':
        return GameState(
            asteroids=[asteroid.copy() for asteroid in self.asteroids if asteroid.alive], 
            ships=[ship.copy() for ship in self.ships], 
            bullets=[bullet.copy() for bullet in self.bullets if bullet.alive], 
            mines=[mine.copy() for mine in self.mines if mine.alive], 
...
'''
def create_game_state_from_dict_OLD(game_state_dict: GameStateDict) -> GameState:
...
def create_asteroid_from_dict(d: AsteroidDict) -> Asteroid:
    x, y = d['position']
    vx, vy = d['velocity']
    return Asteroid(
        x=x,
        ...
        mass=d['mass'],
        radius=d['radius'],
        #timesteps_until_appearance=d['timesteps_until_appearance']
    )

def create_ship_from_dict(d: ShipDict) -> Ship:
    x, y = d.get('position', (0.0, 0.0))
    vx, vy = d.get('velocity', (0.0, 0.0))
    thrust_range = d.get('thrust_range', (-SHIP_MAX_THRUST, SHIP_MAX_THRUST))
    turn_rate_range = d.get('turn_rate_range', (-SHIP_MAX_TURN_RATE, SHIP_MAX_TURN_RATE))
    return Ship(
        is_respawning=d.get('is_respawning', False),
        x=x,
        y=y,
        vx=vx,
        vy=vy,
        ...
        id=d.get('id', 0),
        team=d.get('team', ""),
        lives_remaining=d.get('lives_remaining', 0),
        ...
        drag=d.get('drag', SHIP_DRAG),
    )

def create_mine_from_dict(d: MineDict) -> Mine:
    x, y = d['position']
    return Mine(
        x=x,
        y=y,
        mass=d['mass'],
        fuse_time=d['fuse_time'],
...
def create_game_state_from_dict(game_state_dict: GameStateDict) -> GameState:
    asteroids = [create_asteroid_from_dict(asteroid) for asteroid in game_state_dict['asteroids']]
    ships = [create_ship_from_dict(ship) for ship in game_state_dict['ships']]
    bullets = [create_bullet_from_dict(bullet) for bullet in game_state_dict['bullets']]
    mines = [create_mine_from_dict(mine) for mine in game_state_dict['mines']]
    map_size_x = game_state_dict['map_size'][0]
    map_size_y = game_state_dict['map_size'][1]
    time = game_state_dict['time']
    delta_time = game_state_dict['delta_time']
    sim_frame = game_state_dict['sim_frame']
    time_limit = game_state_dict['time_limit']
    return GameState(asteroids=asteroids, ships=ships, bullets=bullets, mines=mines, map_size_x=map_size_x, map_size_y=map_size_y, time=time, delta_time=delta_time, sim_frame=sim_frame, time_limit=time_limit)
...
def check_mine_opportunity(ship_state: Ship, game_state: GameState, other_ships: list[Ship]) -> bool:
    if len(game_state.mines) > 1:
        return False
    mine_ast_count = count_asteroids_in_mine_blast_radius(game_state, ship_state.x, ship_state.y, round(MINE_FUSE_TIME*FPS))
    lives_fudge: i64 = 0
    for other_ship in other_ships:
        ...
        delta_x = ship_state.x - other_ship.x
        delta_y = ship_state.y - other_ship.y
...
def preprocess_bullets(bullets: list[Bullet]) -> list[Bullet]:
    for b in bullets:
        b.tail_delta_x = -BULLET_LENGTH*cos(b.heading*DEG_TO_RAD)
        b.tail_delta_y = -BULLET_LENGTH*sin(b.heading*DEG_TO_RAD)
    return bullets
...
def inspect_scenario(game_state: GameState, ship_state: Ship) -> None:
    ...
    width = game_state.map_size_x
    height = game_state.map_size_y
...
class GameStatePlotter:
    ...
    def __init__(self, game_state: GameState) -> None:
        ...
        self.ax.set_xlim((0.0, game_state.map_size_x))
        self.ax.set_ylim((0.0, game_state.map_size_y))
    ...
    def update_plot(self, asteroids: Optional[list[Asteroid]] = None, ship_state: Optional[Ship] = None, bullets: Optional[list[Bullet]] = None, special_bullets: Optional[list[Bullet]] = None, circled_asteroids: Optional[list[Asteroid]] = None, ghost_asteroids: Optional[list[Asteroid]] = None, forecasted_asteroids: Optional[list[Asteroid]] = None, mines: Optional[list[Mine]] = None, clear_plot: bool = True, pause_time: float = EPS, plot_title: str = '') -> None:
        ...
        self.ax.set_xlim((0.0, self.game_state.map_size_x))
        self.ax.set_ylim((0.0, self.game_state.map_size_y))
        self.ax.set_aspect('equal', adjustable='box')
        ...
        for a in asteroids:
            if a:
                asteroid_circle = patches.Circle((a.x, a.y), a.radius, color='gray', fill=True)
                self.ax.add_patch(asteroid_circle)
                self.ax.arrow(a.x, a.y, a.vx*DELTA_TIME, a.vy*DELTA_TIME, head_width=3, head_length=5, fc='white', ec='white')
        for a in ghost_asteroids:
            if a:
                asteroid_circle = patches.Circle((a.x, a.y), a.radius, color='#333333', fill=True, zorder=-100)
                self.ax.add_patch(asteroid_circle)
                # self.ax.arrow(a.x, a.y, a.vx*delta_time, a.vy*delta_time, head_width=3, head_length=5, fc='white', ec='white')
        for a in forecasted_asteroids:
            if a:
                asteroid_circle = patches.Circle((a.x, a.y), a.radius, color='#440000', fill=True, zorder=100, alpha=0.4)
                self.ax.add_patch(asteroid_circle)
        for a in circled_asteroids:
            if a:
                highlight_circle = patches.Circle((a.x, a.y), a.radius + 5, color='orange', fill=False)
                self.ax.add_patch(highlight_circle)

        for m in mines:
            if m:
                highlight_circle = patches.Circle((m.x, m.y), MINE_BLAST_RADIUS, color=self.fuse_time_to_color(m.remaining_time), fill=False)
                self.ax.add_patch(highlight_circle)
        ...
        if ship_state:
            ...
            ship_position = (ship_state.x, ship_state.y)
            angle_rad = ship_heading*DEG_TO_RAD
            ship_vertices = [
        ...
        for b in bullets:
            if b:
                bullet_tail = (b.x - BULLET_LENGTH*cos(b.heading*DEG_TO_RAD), b.y - BULLET_LENGTH*sin(b.heading*DEG_TO_RAD))
                self.ax.arrow(bullet_tail[0], bullet_tail[1], b.x - bullet_tail[0], b.y - bullet_tail[1], head_width=3, head_length=5, fc='red', ec='red')
        for b in special_bullets:
            if b:
                bullet_tail = (b.x - BULLET_LENGTH*cos(b.heading*DEG_TO_RAD), b.y - BULLET_LENGTH*sin(b.heading*DEG_TO_RAD))
                self.ax.arrow(bullet_tail[0], bullet_tail[1], b.x - bullet_tail[0], b.y - bullet_tail[1], head_width=3, head_length=5, fc='green', ec='green')
        plt.draw()
...
def get_ship_maneuver_move_sequence(ship_heading_angle: float, ship_cruise_speed: float, ship_accel_turn_rate: float, ship_cruise_timesteps: i64, ship_cruise_turn_rate: float, ship_starting_speed: float = 0.0) -> list[Action]:
    ...
    flag = False
    for a in move_sequence:
        if math.isclose(a.turn_rate, 81.69680070842742):
            flag = True
    if math.isclose(ship_accel_turn_rate, 81.69680070842742):
        flag = True
    if flag:
        print()
        print(f"FOUND THE BAD MOVE SEQUENCE doing rotate to {ship_heading_angle=} accelerate to {ship_cruise_speed=} at turn rate of {ship_accel_turn_rate=} and cruise for {ship_cruise_timesteps=} at tr of {ship_cruise_turn_rate=}")
        print(move_sequence)
        print()
...
def analyze_gamestate_for_heuristic_maneuver(game_state: GameState, ship_state: Ship) -> tuple[float, float, float, float, i64, float, i64, i64]:
    ...
    for ship in other_ships:
        ...
        asteroids.append(Asteroid(x=ship.x, y=ship.y, vx=0.0, vy=0.0, radius=ship.radius, size=-1, mass=-1.0))
    ship_pos_x, ship_pos_y, ship_vel_x, ship_vel_y = ship_state.x, ship_state.y, ship_state.vx, ship_state.vy
    ...
    for asteroid in asteroids:
        for a in unwrap_asteroid(asteroid, game_state.map_size_x, game_state.map_size_y, UNWRAP_ASTEROID_COLLISION_FORECAST_TIME_HORIZON, False):
            ...
            delta_x = a.x - ship_pos_x
            delta_y = a.y - ship_pos_y
            asteroid_speed = None
    ...
    if most_imminent_asteroid is None:
        ...
        imminent_asteroid_relative_heading_deg = degrees(super_fast_atan2(most_imminent_asteroid.y - ship_pos_y, most_imminent_asteroid.x - ship_pos_x))
...
def predict_next_imminent_collision_time_with_asteroid(ship_pos_x: float, ship_pos_y: float, ship_vel_x: float, ship_vel_y: float, ship_r: float, ast_pos_x: float, ast_pos_y: float, ast_vel_x: float, ast_vel_y: float, ast_radius: float, game_state: GameState) -> float:
    ...
    if isnan(start_collision_time) or isnan(end_collision_time):
        ...
        if not (SHIP_RADIUS_PLUS_SIZE_4_ASTEROID_RADIUS < ship_pos_x < game_state.map_size_x - SHIP_RADIUS_PLUS_SIZE_4_ASTEROID_RADIUS and SHIP_RADIUS_PLUS_SIZE_4_ASTEROID_RADIUS < ship_pos_y < game_state.map_size_y - SHIP_RADIUS_PLUS_SIZE_4_ASTEROID_RADIUS):
...
def unwrap_asteroid(asteroid: Asteroid, max_x: float, max_y: float, time_horizon_s: float = 10.0, use_cache: bool = True) -> list[Asteroid]:
    ...
    #hash_tuple = (asteroid.x, asteroid.y, asteroid.vx, asteroid.vy, time_horizon_s)
    if use_cache:
    ...
    #border_crossings = calculate_border_crossings(asteroid.x, asteroid.y, asteroid.vx, asteroid.vy, max_x, max_y, time_horizon_s)
    for universe in calculate_border_crossings(asteroid.x, asteroid.y, asteroid.vx, asteroid.vy, max_x, max_y, time_horizon_s):
        ...
        #unwrapped_asteroid.position = (unwrapped_asteroid.x + dx, unwrapped_asteroid.y + dy)
...
def check_coordinate_bounds(game_state: GameState, x: float, y: float) -> bool:
    if 0.0 <= x <= game_state.map_size_x and 0.0 <= y <= game_state.map_size_y:
        return True
    else:
        return False
...
def check_coordinate_bounds_exact(game_state: GameState, x: float, y: float) -> bool:
    x_wrapped = x % game_state.map_size_x
    y_wrapped = y % game_state.map_size_y
    # TODO: Maybe optimize?
...
def calculate_interception(ship_pos_x: float, ship_pos_y: float, asteroid_pos_x: float, asteroid_pos_y: float, asteroid_vel_x: float, asteroid_vel_y: float, asteroid_r: float, ship_heading_deg: float, game_state: GameState, future_shooting_timesteps: i64 = 0) -> tuple[bool, float, float, float, float, float, float]:
    ...
    vb = BULLET_SPEED
    vb_sq = vb*vb
    theta_0 = ship_heading_deg*DEG_TO_RAD
...
def forecast_asteroid_bullet_splits_from_heading(a: Asteroid, timesteps_until_appearance: i64, bullet_heading_deg: float, game_state: GameState) -> tuple[Asteroid, Asteroid, Asteroid]:
    # assert a.size != 1 # Asteroids of size 1 don't split  # REMOVE_FOR_COMPETITION
    # Look at asteroid.py in the Kessler game's code
    bullet_heading_rad = bullet_heading_deg*DEG_TO_RAD
...
def forecast_instantaneous_asteroid_bullet_splits_from_velocity(a: Asteroid, bullet_vx: float, bullet_vy: float, game_state: GameState) -> tuple[Asteroid, Asteroid, Asteroid]:
    # assert a.size != 1 # Asteroids of size 1 don't split  # REMOVE_FOR_COMPETITION
    # Look at asteroid.py in the Kessler game's code
    vfx = (1.0/(BULLET_MASS + a.mass))*(BULLET_MASS*bullet_vx + a.mass*a.vx)
    vfy = (1.0/(BULLET_MASS + a.mass))*(BULLET_MASS*bullet_vy + a.mass*a.vy)
    v = sqrt(vfx*vfx + vfy*vfy)
    return forecast_asteroid_splits(a, 0, vfx, vfy, v, 15.0, game_state)


def forecast_asteroid_mine_instantaneous_splits(asteroid: Asteroid, mine: Mine, game_state: GameState) -> tuple[Asteroid, Asteroid, Asteroid]:
    # assert asteroid.size != 1 # Asteroids of size 1 don't split  # REMOVE_FOR_COMPETITION
    delta_x = mine.x - asteroid.x
    delta_y = mine.y - asteroid.y
    dist = sqrt(delta_x*delta_x + delta_y*delta_y)
    F = (-dist/MINE_BLAST_RADIUS + 1.0)*MINE_BLAST_PRESSURE*2.0*asteroid.radius
    a = F/asteroid.mass
    # calculate "impulse" based on acc
    if dist != 0.0:
        cos_theta = (asteroid.x - mine.x)/dist
        sin_theta = (asteroid.y - mine.y)/dist
...
def forecast_asteroid_ship_splits(asteroid: Asteroid, timesteps_until_appearance: i64, ship_vx: float, ship_vy: float, game_state: GameState) -> tuple[Asteroid, Asteroid, Asteroid]:
    # assert asteroid.size != 1 # Asteroids of size 1 don't split  # REMOVE_FOR_COMPETITION
    vfx = (1.0/(SHIP_MASS + asteroid.mass))*(SHIP_MASS*ship_vx + asteroid.mass*asteroid.vx)
    vfy = (1.0/(SHIP_MASS + asteroid.mass))*(SHIP_MASS*ship_vy + asteroid.mass*asteroid.vy)
    v = sqrt(vfx*vfx + vfy*vfy)
    return forecast_asteroid_splits(asteroid, timesteps_until_appearance, vfx, vfy, v, 15.0, game_state)


def forecast_asteroid_splits(a: Asteroid, timesteps_until_appearance: i64, vfx: float, vfy: float, v: float, split_angle: float, game_state: GameState) -> tuple[Asteroid, Asteroid, Asteroid]:
    # Calculate angle of center asteroid for split (degrees)
    theta = atan2(vfy, vfx)*RAD_TO_DEG # DO NOT USE AN APPROXIMATION FOR ATAN2!! This needs to match Kessler or else we can get desyncs.
    ...
    angle_left = (theta + split_angle)*DEG_TO_RAD
    angle_center = theta*DEG_TO_RAD
    angle_right = (theta - split_angle)*DEG_TO_RAD

    cos_angle_left = cos(angle_left)
    sin_angle_left = sin(angle_left)
    cos_angle_center = cos(angle_center)
    sin_angle_center = sin(angle_center)
    cos_angle_right = cos(angle_right)
    sin_angle_right = sin(angle_right)
    ...
    if timesteps_until_appearance == 0:
        return (
            ...
            Asteroid(
                x=a.x,
                y=a.y,
                vx=v*cos_angle_center,
                vy=v*sin_angle_center,
                size=new_size,
                mass=new_mass,
                radius=new_radius,
                timesteps_until_appearance=0
            ...
            Asteroid(
                x=a.x,
                y=a.y,
                vx=v*cos(angle_right),
                vy=v*sin(angle_right),
                size=new_size,
                mass=new_mass,
                radius=new_radius,
                timesteps_until_appearance=0
        ...
        dt = DELTA_TIME*float(timesteps_until_appearance)
        return (
            Asteroid(
                x=(a.x + a.vx*DELTA_TIME*float(timesteps_until_appearance) - dt*cos_angle_left*v) % game_state.map_size_x, 
                y=(a.y + a.vy*DELTA_TIME*float(timesteps_until_appearance) - dt*sin_angle_left*v) % game_state.map_size_y,
                vx=v*cos_angle_left,
                vy=v*sin_angle_left,
                size=new_size,
                mass=new_mass,
            ...
            Asteroid(
                x=(a.x + a.vx*DELTA_TIME*float(timesteps_until_appearance) - dt*cos_angle_center*v) % game_state.map_size_x, 
                y=(a.y + a.vy*DELTA_TIME*float(timesteps_until_appearance) - dt*sin_angle_center*v) % game_state.map_size_y,
                vx=v*cos_angle_center,
                vy=v*sin_angle_center,
                size=new_size,
                mass=new_mass,
            ...
            Asteroid(
                x=(a.x + a.vx*DELTA_TIME*float(timesteps_until_appearance) - dt*cos_angle_right*v) % game_state.map_size_x, 
                y=(a.y + a.vy*DELTA_TIME*float(timesteps_until_appearance) - dt*sin_angle_right*v) % game_state.map_size_y,
                vx=v*cos_angle_right,
                vy=v*sin_angle_right,
                size=new_size,
                mass=new_mass,
                radius=new_radius,
                timesteps_until_appearance=timesteps_until_appearance
...
def forecast_asteroid_splits_OLD(a: Asteroid, timesteps_until_appearance: i64, vfx: float, vfy: float, v: float, split_angle: float, game_state: GameState) -> list[Asteroid]:
    # Calculate angle of center asteroid for split (degrees)
    theta = atan2(vfy, vfx)*RAD_TO_DEG # DO NOT USE AN APPROXIMATION FOR ATAN2!! This needs to match Kessler or else we can get desyncs.
    # Split angle is the angle off of the new velocity vector for the two asteroids to the sides, the center child asteroid continues on the new velocity path
    angles = ((theta + split_angle)*DEG_TO_RAD, theta*DEG_TO_RAD, (theta - split_angle)*DEG_TO_RAD)
    # We redundantly convert to degrees, add split angle, and back to radians. But it has to do this to match Kessler. No way to optimize without risking desyncs.
    # This is wacky because we're back-extrapolation the position of the asteroid BEFORE IT WAS BORN!!!!11!
    new_size = a.size - 1
    new_mass = ASTEROID_MASS_LOOKUP[new_size]
    ...
    if timesteps_until_appearance == 0:
        ...
        return [
            Asteroid(
                x=(a.x + a.vx*DELTA_TIME*float(timesteps_until_appearance) - float(timesteps_until_appearance)*cos(angle)*v*DELTA_TIME) % game_state.map_size_x, 
                y=(a.y + a.vy*DELTA_TIME*float(timesteps_until_appearance) - float(timesteps_until_appearance)*sin(angle)*v*DELTA_TIME) % game_state.map_size_y,
                vx=v*cos(angle),
                vy=v*sin(angle),
                size=new_size,
                mass=new_mass,
...
def maintain_forecasted_asteroids(forecasted_asteroid_splits: list[Asteroid], game_state: GameState) -> list[Asteroid]:
    ...
    updated_asteroids = [
        Asteroid(
            x=(forecasted_asteroid.x + forecasted_asteroid.vx * DELTA_TIME) % game_state.map_size_x,
            y=(forecasted_asteroid.y + forecasted_asteroid.vy * DELTA_TIME) % game_state.map_size_y,
...
def is_asteroid_in_list(list_of_asteroids: list[Asteroid], asteroid: Asteroid, game_state: GameState) -> bool:
    ...
    for a in list_of_asteroids:
        ...
        if (is_close(a.x, asteroid.x) or is_close_to_zero(game_state.map_size_x - abs(a.x - asteroid.x))) and (is_close(a.y, asteroid.y) or is_close_to_zero(game_state.map_size_y - abs(a.y - asteroid.y))) and is_close(a.vx, asteroid.vx) and is_close(a.vy, asteroid.vy) and a.size == asteroid.size:
            #assert is_close(a.x, asteroid.x) and is_close(a.y, asteroid.y) and a.vx == asteroid.vx and a.vy == asteroid.vy and a.size == asteroid.size
            return True
    return False
...
def predict_ship_mine_collision(ship_pos_x: float, ship_pos_y: float, mine: Mine, future_timesteps: i64 = 0) -> float:
    ...
    if mine.remaining_time >= float(future_timesteps)*DELTA_TIME:
        #if check_collision(ship_pos_x, ship_pos_y, SHIP_RADIUS, mine.x, mine.y, MINE_BLAST_RADIUS):
        delta_x = ship_pos_x - mine.x
        delta_y = ship_pos_y - mine.y
...
def asteroid_bullet_collision(bullet_head_x: float, bullet_head_y: float, bullet_tail_x: float, bullet_tail_y: float, asteroid_x: float, asteroid_y: float, asteroid_radius: float) -> bool:
    ...
    bhdx = bullet_head_x - asteroid_x
    bhdy = bullet_head_y - asteroid_y
    a = sqrt(bhdx*bhdx + bhdy*bhdy)
    # b = dist(asteroid_x, asteroid_y, bullet_tail_x, bullet_tail_y)
    btdx = bullet_tail_x - asteroid_x
    btdy = bullet_tail_y - asteroid_y
    b = sqrt(btdx*btdx + btdy*btdy)
    #c = BULLET_LENGTH
...
def get_simulated_ship_max_range(max_cruise_seconds: float) -> tuple[float, i64]:
    ...
    ship_random_range = dist(state_sequence[0].ship_state.x, state_sequence[0].ship_state.y, state_sequence[-1].ship_state.x, state_sequence[-1].ship_state.y)
...
def simulate_ship_movement_with_inputs(game_state: GameState, ship_state: Ship, move_sequence: list[Action]) -> Ship:
    ...
    dummy_game_state = cast(GameState, {'asteroids': [], 'ships': [], 'bullets': [], 'mines': [], 'map_size': (game_state.map_size_x, game_state.map_size_y), 'time': 0.0, 'delta_time': 1/30, 'sim_frame': 0})
...
def solve_interception(asteroid: Asteroid, ship_state: Ship, game_state: GameState, timesteps_until_can_fire: i64 = 0) -> tuple[bool, float, i64, float, float, float, float]:
    ...
    t_0 = 0.0175 # t_0 = (SHIP_RADIUS - 0.5*BULLET_LENGTH)/BULLET_SPEED
    # Positions are relative to the ship. We set the origin to the ship's position. Remember to translate back!
    ship_position_x: float = ship_state.x
    ship_position_y: float = ship_state.y
    origin_x: float = ship_position_x
    origin_y: float = ship_position_y
    avx: float = asteroid.vx
    avy: float = asteroid.vy
    ax: float = asteroid.x - origin_x + avx*DELTA_TIME  # We project the asteroid one timestep ahead, since by the time we shoot our bullet, the asteroid would have moved one more timestep!
    ay: float = asteroid.y - origin_y + avy*DELTA_TIME

    vb = BULLET_SPEED
    vb_sq = vb*vb
    theta_0 = ship_state.heading*DEG_TO_RAD
    ...
    def naive_desired_heading_calc(timesteps_until_fire: i64 = 0) -> tuple[float, float, i64, float, float, float]:
        ...
        for t in solve_quadratic(a, b, c):
            ...
            return t, angle_difference_rad(theta, theta_0), timesteps_until_fire, intercept_x, intercept_y, dist(ship_position_x, ship_position_y, intercept_x, intercept_y)
        #if len(solutions) > 1:
        #    print(len(solutions))
        #return solutions
    ...
    if abs(naive_solution[1]) <= amount_we_can_turn_before_we_can_shoot_rad + EPS:
        ...
        if check_coordinate_bounds(game_state, naive_solution[3], naive_solution[4]):
            ...
            return True, naive_solution[1]*RAD_TO_DEG, timesteps_until_can_fire, naive_solution[0], naive_solution[3], naive_solution[4], naive_solution[5]
    else:
        ...
        delta_theta_solution_deg = delta_theta_solution*RAD_TO_DEG
        ...
        if check_coordinate_bounds(game_state, intercept_x, intercept_y):
            ...
            if not isnan(discrete_solution[0]):
                if not abs(discrete_solution[1]*RAD_TO_DEG) - EPS <= float(t_rot_ts)*SHIP_MAX_TURN_RATE_DEG_TS:
                    return False, nan, -1, nan, nan, nan, nan
                ...
                if check_coordinate_bounds(game_state, discrete_solution[3], discrete_solution[4]):
                    ...
                    return True, discrete_solution[1]*RAD_TO_DEG, t_rot_ts, discrete_solution[0], discrete_solution[3], discrete_solution[4], discrete_solution[5]
...
def track_asteroid_we_shot_at(asteroids_pending_death: dict[i64, list[Asteroid]], current_timestep: i64, game_state: GameState, bullet_travel_timesteps: i64, original_asteroid: Asteroid) -> None:
    ...
    asteroid.x = asteroid.x % game_state.map_size_x
    asteroid.y = asteroid.y % game_state.map_size_y
    ...
    for future_timesteps in range(0, bullet_travel_timesteps + 1):
        ...
        if future_timesteps != bullet_travel_timesteps:
            ...
            asteroid.x = (asteroid.x + asteroid.vx*DELTA_TIME) % game_state.map_size_x
            asteroid.y = (asteroid.y + asteroid.vy*DELTA_TIME) % game_state.map_size_y
...
def check_whether_this_is_a_new_asteroid_for_which_we_do_not_have_a_pending_shot(asteroids_pending_death: dict[i64, list[Asteroid]], current_timestep: i64, game_state: GameState, asteroid: Asteroid) -> bool:
    ...
    def verify_asteroid_does_not_appear_in_wrong_timestep(a: Asteroid) -> bool:  # REMOVE_FOR_COMPETITION
        ...
        for timestep, asts_list in asteroids_pending_death.items():  # REMOVE_FOR_COMPETITION
            if is_asteroid_in_list(asts_list, a, game_state):  # REMOVE_FOR_COMPETITION
                ...
                if abs(delta) <= 120 and not ((is_close_to_zero(float(abs(delta))*DELTA_TIME*a.vx % game_state.map_size_x) and is_close_to_zero(float(abs(delta))*DELTA_TIME*a.vy % game_state.map_size_y)) or is_close(float(abs(delta))*DELTA_TIME*a.vx % game_state.map_size_x, game_state.map_size_x) or is_close(float(abs(delta))*DELTA_TIME*a.vy % game_state.map_size_y, game_state.map_size_y)):  # REMOVE_FOR_COMPETITION
    ...
    #asteroid.position = (asteroid.x % game_state.map_size_x, asteroid.y % game_state.map_size_y)
    if ENABLE_SANITY_CHECKS:  # REMOVE_FOR_COMPETITION
        assert check_coordinate_bounds(game_state, asteroid.x, asteroid.y) or current_timestep == 0, f"Asteroid out of bounds! {asteroid.x} {asteroid.y}"  # REMOVE_FOR_COMPETITION
        if not check_coordinate_bounds(game_state, asteroid.x, asteroid.y):  # REMOVE_FOR_COMPETITION
...
def time_travel_asteroid(asteroid: Asteroid, timesteps: i64, game_state: GameState) -> Asteroid:
    ...
    return Asteroid(
        x=(asteroid.x + float(timesteps)*asteroid.vx*DELTA_TIME) % game_state.map_size_x,
        y=(asteroid.y + float(timesteps)*asteroid.vy*DELTA_TIME) % game_state.map_size_y,
        vx=asteroid.vx,
        vy=asteroid.vy,
        size=asteroid.size,
        mass=asteroid.mass,
...
def time_travel_asteroid_s(asteroid: Asteroid, time: float, game_state: GameState) -> Asteroid:
    ...
    # The ts=0 shortcut isn't good because it doesn't wrap the asteroid!
        ...
        #return asteroid.copy()
    return Asteroid(
        x=(asteroid.x + time*asteroid.vx) % game_state.map_size_x,
        y=(asteroid.y + time*asteroid.vy) % game_state.map_size_y,
        vx=asteroid.vx,
        vy=asteroid.vy,
        size=asteroid.size,
        mass=asteroid.mass,
...
class Matrix():
    ...
    def get_instantaneous_asteroid_collision(self, asteroids: Optional[list[Asteroid]] = None, ship_position: Optional[tuple[float, float]] = None) -> bool:
        ...
        if ship_position is not None:
            position_x = ship_position[0]
            position_y = ship_position[1]
        else:
            position_x = self.ship_state.x
            position_y = self.ship_state.y
    ...
    def get_instantaneous_ship_collision(self) -> bool:
        ...
        for ship in self.other_ships:
            ...
            if check_collision(self.ship_state.x, self.ship_state.y, SHIP_RADIUS, ship.x, ship.y, ship.radius + SHIP_AVOIDANCE_PADDING + sqrt(ship.vx**2 + ship.vy**2)*SHIP_AVOIDANCE_SPEED_PADDING_RATIO):
                return True
        return False
    ...
    def get_instantaneous_mine_collision(self) -> bool:
        ...
        for i, m in enumerate(self.game_state.mines):
            if m.remaining_time < EPS:
                if check_collision(self.ship_state.x, self.ship_state.y, SHIP_RADIUS, m.x, m.y, MINE_BLAST_RADIUS):
                    mine_collision = True
    ...
    def get_next_extrapolated_asteroid_collision_time(self, additional_timesteps_to_blow_up_mines: i64 = 0) -> float:
        ...
        for ast_idx, asteroid in enumerate(chain(self.game_state.asteroids, self.forecasted_asteroid_splits)):
            if asteroid.alive:
                ...
                for a in unwrap_asteroid(asteroid, self.game_state.map_size_x, self.game_state.map_size_y, UNWRAP_ASTEROID_COLLISION_FORECAST_TIME_HORIZON, False):
                    # if self.future_timesteps >= self.timesteps_to_not_check_collision_for:
                    assert is_close_to_zero(self.ship_state.vx) and is_close_to_zero(self.ship_state.vy), f"{self.ship_state.vx=}, {self.ship_state.vy=}, {self.ship_state.speed=}"  # REMOVE_FOR_COMPETITION

                    predicted_collision_time_from_future = predict_next_imminent_collision_time_with_asteroid(self.ship_state.x, self.ship_state.y, self.ship_state.vx, self.ship_state.vy, SHIP_RADIUS, a.x, a.y, a.vx, a.vy, a.radius, self.game_state)
                    if asteroid_is_born:
                        predicted_collision_time = predicted_collision_time_from_future + DELTA_TIME*float(additional_timesteps_to_blow_up_mines)
                    else:
    ...
    def get_next_extrapolated_mine_collision_times_and_pos(self) -> list[tuple[float, tuple[float, float]]]:
        ...
        for m in self.game_state.mines:
            ...
            if not isinf(next_imminent_mine_collision_time):
                times_and_mine_pos.append((next_imminent_mine_collision_time, (m.x, m.y)))
    ...
    def coordinates_in_same_wrap(self, pos1: tuple[float, float], pos2: tuple[float, float]) -> bool:
        ...
        max_width = self.game_state.map_size_x
        max_height = self.game_state.map_size_y
    ...
    def get_fitness(self) -> float:
        ...
        assert is_close_to_zero(self.ship_state.speed), f"In get fitness, the ship speed is not zero: {self.ship_state.speed}, vel is {self.ship_state.vx} {self.ship_state.vy}, current ts is {self.initial_timestep + self.future_timesteps} and the time limit is {self.game_state.time_limit} which is {floor(FPS*self.game_state.time_limit)} frames, {self.sim_id=}, {self.intended_move_sequence=}, {self.ship_move_sequence=}"  # REMOVE_FOR_COMPETITION
        ...
        def get_mine_safety_fitness(next_extrapolated_mine_collision_times: list[tuple[float, tuple[float, float]]]) -> tuple[float, float]:
            ...
            if next_extrapolated_mine_collision_times:
                for mine_collision_time, mine_pos in next_extrapolated_mine_collision_times:
                    ...
                    dist_to_ground_zero = dist(self.ship_state.x, self.ship_state.y, mine_pos[0], mine_pos[1])
        ...
        def get_asteroid_aiming_cone_fitness() -> float:
            ...
            ship_heading_rad = self.ship_state.heading*DEG_TO_RAD
            asts_within_cone = 0
            for a in chain(self.game_state.asteroids, self.forecasted_asteroid_splits):
                if a.alive:
                    ...
                    if heading_diff_within_threshold(ship_heading_rad, a.x - self.ship_state.x, a.y - self.ship_state.y, AIMING_CONE_FITNESS_CONE_WIDTH_HALF_COSINE):
                        asts_within_cone += 1
        ...
        def get_other_ship_proximity_fitness(self_positions: list[tuple[float, float]]) -> float:
            ...
            for other_ship in self.other_ships:
                ...
                other_ship_speed = sqrt(other_ship.vx*other_ship.vx + other_ship.vy*other_ship.vy)
                ...
                for self_pos in self_positions:
                    ...
                    abs_sep_x = abs(self_pos_x - other_ship.x)
                    abs_sep_y = abs(self_pos_y - other_ship.y)
                    sep_x = min(abs_sep_x, self.game_state.map_size_x - abs_sep_x)
                    sep_y = min(abs_sep_y, self.game_state.map_size_y - abs_sep_y)
        ...
        # Cull stuff
        self.game_state.mines = [m for m in self.game_state.mines if m.alive]
        ...
        if self.game_state.mines:
            ...
            self.backed_up_game_state_before_post_mutation.asteroids = [a.copy() for a in self.game_state.asteroids if a.alive]
            self.backed_up_game_state_before_post_mutation.mines = [m.copy() for m in self.game_state.mines if m.alive]
            self.backed_up_game_state_before_post_mutation.bullets = [b.copy() for b in self.game_state.bullets if b.alive]
        ...
        ship_start_position_x = states[0].ship_state.x
        ship_start_position_y = states[0].ship_state.y
        ship_end_position_x = states[-1].ship_state.x
        ship_end_position_y = states[-1].ship_state.y
        ...
        if displacement < EPS or self.respawn_maneuver_pass_number > 0:
            ...
            self_ship_positions = [(s.ship_state.x, s.ship_state.y) for s in states]
    ...
    def target_selection(self) -> bool:
        ...
        if not actual_asteroid_hit:
            ...
            if target_asteroids_list:
                ...
                assert is_close_to_zero(self.ship_state.speed), f"When returning in target selection where the return value of whether we didn't crash is {sim_complete_without_crash}, the ship speed is not zero! {self.ship_state.speed=}, {self.ship_state.vx=}, {self.ship_state.vy=}"  # REMOVE_FOR_COMPETITION
        ...
        if actual_asteroid_hit is None or not check_whether_this_is_a_new_asteroid_for_which_we_do_not_have_a_pending_shot(self.asteroids_pending_death, self.initial_timestep + self.future_timesteps + len(aiming_move_sequence), self.game_state, actual_asteroid_hit_when_firing):
            ...
            assert is_close_to_zero(self.ship_state.speed), f"When returning in target sel where the return value of whether we didn't crash is {sim_complete_without_crash}, the ship speed is not zero! {self.ship_state.speed=}, {self.ship_state.vx=}, {self.ship_state.vy=}"  # REMOVE_FOR_COMPETITION
            return sim_complete_without_crash
        else:
            ...
            #debug_print(f"We used a sim to forecast the asteroid that we'll hit being the one at pos ({actual_asteroid_hit.x}, {actual_asteroid_hit.y}) with vel ({actual_asteroid_hit.vx}, {actual_asteroid_hit.vy}) in {timesteps_until_bullet_hit_asteroid - len(aiming_move_sequence)} timesteps")
            #debug_print(f"The primitive forecast would have said we'd hit asteroid at pos ({target_asteroid.x}, {target_asteroid.y}) with vel ({target_asteroid.vx}, {target_asteroid.vy}) in {calculate_timesteps_until_bullet_hits_asteroid(target_asteroid_interception_time_s, target_asteroid.radius)} timesteps")
            # print(self.asteroids_pending_death)
            ...
            #print("applying move seq from 4264")
            ...
            assert is_close_to_zero(self.ship_state.speed), f"When returning in target selection's end where the return value is {sim_complete_without_crash}, the ship speed is not zero! {self.ship_state.speed=}, {self.ship_state.vx=}, {self.ship_state.vy=}"  # REMOVE_FOR_COMPETITION
            return sim_complete_without_crash
    ...
    def bullet_sim(self, ship_state: Optional[Ship] = None, fire_first_timestep: bool = False, fire_after_timesteps: i64 = 0, skip_half_of_first_cycle: bool = False, current_move_index: Optional[i64] = None, whole_move_sequence: Optional[list[Action]] = None, timestep_limit: i64 = INT_INF, asteroids_to_check: Optional[list[Asteroid]] = None) -> tuple[Optional[Asteroid], i64, bool]:
        ...
        if asteroids_to_check is not None:
            asteroids = cast(list[Asteroid], [a.copy() for a in asteroids_to_check if a.alive])
        else:
            asteroids = cast(list[Asteroid], [a.copy() for a in self.game_state.asteroids if a.alive])
        mines: list[Mine] = cast(list[Mine], [m.copy() for m in self.game_state.mines if m.alive])
        bullets: list[Bullet] = cast(list[Bullet], [b.copy() for b in self.game_state.bullets if b.alive])
        initial_ship_state = self.get_ship_state()
        if ship_state and ENABLE_SANITY_CHECKS:  # REMOVE_FOR_COMPETITION
            assert check_coordinate_bounds(self.game_state, ship_state.x, ship_state.y)  # REMOVE_FOR_COMPETITION
        ...
        while True:
            ...
            if not (skip_half_of_first_cycle and timesteps_until_bullet_hit_asteroid == 0):
                ...
                for b_ind, b in enumerate(bullets):
                    if b.alive:
                        ...
                        if 0.0 <= new_bullet_x <= self.game_state.map_size_x and 0.0 <= new_bullet_y <= self.game_state.map_size_y:
                            b.x = new_bullet_x
                            b.y = new_bullet_y
                        else:
                            b.alive = False
                if my_bullet is not None:
                    my_new_bullet_x = my_bullet.x + my_bullet.vx*DELTA_TIME
                    my_new_bullet_y = my_bullet.y + my_bullet.vy*DELTA_TIME
                    #if check_coordinate_bounds(self.game_state, my_new_bullet_x, my_new_bullet_y):
                    if 0.0 <= my_new_bullet_x <= self.game_state.map_size_x and 0.0 <= my_new_bullet_y <= self.game_state.map_size_y:
                        my_bullet.x = my_new_bullet_x
                        my_bullet.y = my_new_bullet_y
                    else:
                        #bullet_sim_time += time.perf_counter() - start_time
                ...
                for a in asteroids:
                    # Idea: I can advance the asteroid REGARDLESS of whether they're alive or not, and just have ghost asteroids that don't do anything
                    # Might be faster, probably slower, idk, just an idea!
                    if a.alive:
                        a.x = (a.x + a.vx*DELTA_TIME) % self.game_state.map_size_x
                        a.y = (a.y + a.vy*DELTA_TIME) % self.game_state.map_size_y
            ...
            if fire_first_timestep and timesteps_until_bullet_hit_asteroid + (-1 if not skip_half_of_first_cycle else 0) == 0:
                ...
                bullet_x = initial_ship_state.x + SHIP_RADIUS*cos_heading
                bullet_y = initial_ship_state.y + SHIP_RADIUS*sin_heading
                # Make sure the bullet isn't being fired out into the void
                #if check_coordinate_bounds(self.game_state, bullet_x, bullet_y):
                if 0.0 <= bullet_x <= self.game_state.map_size_x and 0.0 <= bullet_y <= self.game_state.map_size_y:
                    initial_timestep_fire_bullet = Bullet(
                        ...
                        mass=BULLET_MASS,
                        tail_delta_x=-BULLET_LENGTH*cos_heading,
                        tail_delta_y=-BULLET_LENGTH*sin_heading
                    )
            ...
            if my_bullet is None and timesteps_until_bullet_hit_asteroid + (-1 if not skip_half_of_first_cycle else 0) == fire_after_timesteps:
                if ship_state is not None:
                    ...
                    bullet_fired_from_ship_position_x = self.ship_state.x
                    bullet_fired_from_ship_position_y = self.ship_state.y
                rad_heading = bullet_fired_from_ship_heading*DEG_TO_RAD
                cos_heading = cos(rad_heading)
                sin_heading = sin(rad_heading)
                bullet_x = bullet_fired_from_ship_position_x + SHIP_RADIUS*cos_heading
                bullet_y = bullet_fired_from_ship_position_y + SHIP_RADIUS*sin_heading
                # Make sure the bullet isn't being fired out into the void
                #if not check_coordinate_bounds(self.game_state, bullet_x, bullet_y):
                if not (0.0 <= bullet_x <= self.game_state.map_size_x and 0.0 <= bullet_y <= self.game_state.map_size_y):
                    #bullet_sim_time += time.perf_counter() - start_time
                    return None, i64(-1), ship_not_collided_with_asteroid  # The bullet got shot into the void without hitting anything :(
                my_bullet = Bullet(
                    x=bullet_x,
            ...
            if whole_move_sequence:
                ...
                if current_move_index + timesteps_until_bullet_hit_asteroid + (-1 if not skip_half_of_first_cycle else 0) < len(whole_move_sequence):
                    ...
                    bullet_sim_ship_state.x = (bullet_sim_ship_state.x + bullet_sim_ship_state.vx*DELTA_TIME) % self.game_state.map_size_x
                    bullet_sim_ship_state.y = (bullet_sim_ship_state.y + bullet_sim_ship_state.vy*DELTA_TIME) % self.game_state.map_size_y

            # Check bullet/asteroid collisions
            len_bullets = len(bullets)
            for b_idx, b in enumerate(chain(bullets, [my_bullet] if my_bullet is not None else [])):
                if b.alive:
                    b_tail_x = b.x + b.tail_delta_x
                    b_tail_y = b.y + b.tail_delta_y
                    for a in asteroids:
                        if a.alive:
                            # If collision occurs
            ...
            if ship_not_collided_with_asteroid:
                if whole_move_sequence is not None:
                    ...
                    ship_position_x = self.ship_state.x
                    ship_position_y = self.ship_state.y
                for asteroid in asteroids:
                    if asteroid.alive:
                        #if check_collision(ship_position[0], ship_position[1], SHIP_RADIUS, asteroid.x, asteroid.y, asteroid.radius):
    ...
    def apply_move_sequence(self, move_sequence: list[Action], allow_free_firing: bool = False, allow_free_mining: bool = False) -> bool:
        if self.sim_id in [24111]:
            print(f"Applying move seq in 24111 {self.ship_state=} {move_sequence=} {allow_free_firing=} {allow_free_mining=}")
        ...
        if ENABLE_SANITY_CHECKS:  # REMOVE_FOR_COMPETITION
            assert is_close_to_zero(self.ship_state.speed), f"When returning in apply move sequence where {sim_was_safe=}, the ship speed is not zero! {self.ship_state.speed=}, {self.ship_state.vx=} {self.ship_state.vy=}. The whole move sequence is {move_sequence}"  # REMOVE_FOR_COMPETITION
        if self.sim_id in [24111]:
            print(f"24111 {sim_was_safe=}")
        return sim_was_safe
    ...
    def simulate_maneuver(self, move_sequence: list[Action], allow_free_firing: bool, allow_free_mining: bool) -> bool:
        ...
        if ENABLE_SANITY_CHECKS:  # REMOVE_FOR_COMPETITION
            assert is_close_to_zero(self.ship_state.speed), f"When returning in simulate maneuver where the sim was safe, the ship speed is not zero! {self.ship_state.speed=}, {self.ship_state.vx=} {self.ship_state.vy=}. The whole move sequence is {move_sequence}"  # REMOVE_FOR_COMPETITION
        return True
    ...
    def update(self, thrust: float = 0.0, turn_rate: float = 0.0, fire: Optional[bool] = None, drop_mine: Optional[bool] = None, whole_move_sequence: Optional[list[Action]] = None, wait_out_mines: bool = False) -> bool:
        ...
        if not wait_out_mines:
            ...
            if ENABLE_SANITY_CHECKS:
                for a in self.forecasted_asteroid_splits_history[-1]:
                    assert a.alive, "Bad assumption, use the alive check below too!"
        ...
        if not wait_out_mines:
                        ...
                        if timesteps_until_can_fire == 0:
                            ...
                            for ast_idx, asteroid in enumerate(chain(self.game_state.asteroids, self.forecasted_asteroid_splits)):
                                if asteroid.alive:
                                    ...
                                    if ast_idx < len_asteroids:
                                        ...
                                        if heading_diff_within_threshold(ship_heading_rad, asteroid.x - self.ship_state.x, asteroid.y - self.ship_state.y, MANEUVER_BULLET_SIM_CULLING_CONE_WIDTH_ANGLE_HALF_COSINE):
                                    ...
                                    if check_whether_this_is_a_new_asteroid_for_which_we_do_not_have_a_pending_shot(self.asteroids_pending_death, self.initial_timestep + self.future_timesteps + 1, self.game_state, asteroid):
                                        for a in unwrap_asteroid(asteroid, self.game_state.map_size_x, self.game_state.map_size_y, UNWRAP_ASTEROID_TARGET_SELECTION_TIME_HORIZON, True):
                                            ...
                                            if not heading_diff_within_threshold(ship_heading_rad, a.x - self.ship_state.x, a.y - self.ship_state.y, MANEUVER_CONVENIENT_SHOT_CHECKER_CONE_WIDTH_ANGLE_HALF_COSINE):
                                                continue
                                            ...
                                            feasible, shot_heading_error_rad, shot_heading_tolerance_rad, interception_time, _, _, _ = calculate_interception(self.ship_state.x, self.ship_state.y, a.x, a.y, a.vx, a.vy, a.radius, self.ship_state.heading, self.game_state)
                                            if feasible:
                            ...
                            if self.respawn_maneuver_pass_number == 0 and (self.future_timesteps >= MANEUVER_SIM_DISALLOW_TARGETING_FOR_START_TIMESTEPS_AMOUNT):# or not fire_this_timestep):
                                ...
                                if not isnan(next_target_heading_error):
                                    min_shot_heading_error_deg = next_target_heading_error*RAD_TO_DEG
                            ...
                            if ship_pred_speed > SHIP_MAX_SPEED:
                                ...
                                ship_pred_speed = -SHIP_MAX_SPEED
                            rad_heading = self.ship_state.heading*DEG_TO_RAD
                            ship_speed_ts = DELTA_TIME*float(timesteps_until_can_fire)*ship_pred_speed
                            ship_predicted_pos_x = self.ship_state.x + ship_speed_ts*cos(rad_heading)
                            ship_predicted_pos_y = self.ship_state.y + ship_speed_ts*sin(rad_heading)
                            for asteroid in chain(self.game_state.asteroids, self.forecasted_asteroid_splits):
                                if asteroid.alive:
                                    # Avoid shooting my size 1 asteroids that are about to get mined by my mine
                                    avoid_targeting_this_asteroid = False
                                    ...
                                    if check_whether_this_is_a_new_asteroid_for_which_we_do_not_have_a_pending_shot(self.asteroids_pending_death, self.initial_timestep + self.future_timesteps + 1, self.game_state, asteroid):
                                        for a in unwrap_asteroid(asteroid, self.game_state.map_size_x, self.game_state.map_size_y, UNWRAP_ASTEROID_TARGET_SELECTION_TIME_HORIZON, True):
                                            ...
                                            if not heading_diff_within_threshold(ship_heading_rad, a.x - self.ship_state.x, a.y - self.ship_state.y, MANEUVER_CONVENIENT_SHOT_CHECKER_CONE_WIDTH_ANGLE_HALF_COSINE):
                                                continue
                                            # feasible, shot_heading_error_rad, shot_heading_tolerance_rad, interception_time, intercept_x, intercept_y, asteroid_dist_during_interception
                                            feasible, shot_heading_error_rad, shot_heading_tolerance_rad, interception_time, _, _, _ = calculate_interception(ship_predicted_pos_x, ship_predicted_pos_y, a.x, a.y, a.vx, a.vy, a.radius, self.ship_state.heading, self.game_state, timesteps_until_can_fire)
                                            if feasible and (self.asteroids_shot >= RANDOM_WALK_SCHEDULE_LENGTH or (self.random_walk_schedule[self.asteroids_shot] and shot_heading_error_rad >= 0.0) or (not self.random_walk_schedule[self.asteroids_shot] and shot_heading_error_rad <= 0.0)):
                                                shot_heading_error_deg = shot_heading_error_rad*RAD_TO_DEG
                                                shot_heading_tolerance_deg = shot_heading_tolerance_rad*RAD_TO_DEG
            ...
            if fire_this_timestep:
                ...
                cos_heading = cos(rad_heading)
                sin_heading = sin(rad_heading)
                bullet_x = self.ship_state.x + SHIP_RADIUS*cos_heading
                bullet_y = self.ship_state.y + SHIP_RADIUS*sin_heading
                # Make sure the bullet isn't being fired out into the void
                #if check_coordinate_bounds(self.game_state, bullet_x, bullet_y):
                if 0.0 <= bullet_x <= self.game_state.map_size_x and 0.0 <= bullet_y <= self.game_state.map_size_y:
                    new_bullet = Bullet(
            ...
            if self.ship_state.mines_remaining != 0 and self.last_timestep_mined <= self.initial_timestep + self.future_timesteps - MINE_COOLDOWN_TS:
                ...
                if drop_mine is None:
                    ...
                    if not isinf(self.game_state.time_limit):
                        ...
                        if not self.halt_shooting and self.initial_timestep + self.future_timesteps + 90 == floor(FPS*self.game_state.time_limit) and count_asteroids_in_mine_blast_radius(self.game_state, self.ship_state.x, self.ship_state.y, round(MINE_FUSE_TIME*FPS)) > 0:
                ...
                if drop_mine_this_timestep:
                    ...
                    self.mine_positions_placed.add((self.ship_state.x, self.ship_state.y)) # Track where we placed our mine
            ...
            # Update the position based off the velocities
            # Do the wrap in the same operation
            self.ship_state.x = (self.ship_state.x + self.ship_state.vx*DELTA_TIME) % self.game_state.map_size_x
            self.ship_state.y = (self.ship_state.y + self.ship_state.vy*DELTA_TIME) % self.game_state.map_size_y
                ...
                if mine.remaining_time < EPS:
                    ...
                    if not wait_out_mines:
                        if not self.ship_state.is_respawning:
                            #if check_collision(self.ship_state.x, self.ship_state.y, SHIP_RADIUS, mine.x, mine.y, MINE_BLAST_RADIUS):
                            delta_x = self.ship_state.x - mine.x
                            delta_y = self.ship_state.y - mine.y
                            separation = SHIP_RADIUS + MINE_BLAST_RADIUS
                            if abs(delta_x) <= separation and abs(delta_y) <= separation and delta_x*delta_x + delta_y*delta_y <= separation*separation:
        ...
        #if mine_remove_idxs:
        #    self.game_state.mines = [mine for idx, mine in enumerate(self.game_state.mines) if idx not in mine_remove_idxs]
        if mine_got_destroyed and wait_out_mines:
            # Cull if we're waiting out mines, and want to see that the list of mines is empty!
            self.game_state.mines = [m for m in self.game_state.mines if m.alive]
        if new_asteroids:
        ...
        if not wait_out_mines:
            ...
            if not self.ship_state.is_respawning:
                ...
                for asteroid in self.game_state.asteroids:
                    if asteroid.alive:
                        #if check_collision(self.ship_state.x, self.ship_state.y, SHIP_RADIUS, asteroid.x, asteroid.y, asteroid.radius):
                        delta_x = self.ship_state.x - asteroid.x
                        delta_y = self.ship_state.y - asteroid.y
                        separation = SHIP_RADIUS + asteroid.radius
                        if abs(delta_x) <= separation and abs(delta_y) <= separation and delta_x*delta_x + delta_y*delta_y <= separation*separation:
                            if asteroid.size != 1:
    ...
    def cruise(self, cruise_time: i64, cruise_turn_rate: float = 0.0) -> bool:
        ...
        for _ in range(cruise_time):
            if self.sim_id == 4271:
                print(f"In respawn sim that'll crash, {self.future_timesteps=} {self.respawn_timer=}, {self.ship_state}")
            if not self.update(sign(self.ship_state.speed)*SHIP_DRAG, cruise_turn_rate):
                if self.sim_id == 4271:
                    print("AHA CRUISE FAILED SOMEHOW?!!!!", self.ship_state, self.respawn_timer_history)
                return False
        return True
    ...
    def get_position(self) -> tuple[float, float]:
        return (self.ship_state.x, self.ship_state.y)
    ...
    def get_velocity(self) -> tuple[float, float]:
        return (self.ship_state.vx, self.ship_state.vy)
...
class NeoController(KesslerController):
    ...
    def decide_next_action_continuous(self, game_state: GameState, ship_state: Ship, force_decision: bool) -> bool:
        ...
        if PLOT_MANEUVER_TRACES:
            ...
            for thing in self.sims_this_planning_period:
                ...
                for state in state_seq:
                    ship_pos = (state.ship_state.x, state.ship_state.y)
        ...
        if self.game_state_to_base_planning['respawning']:
            ...
            #print("applying move seq from 5653")
            #print("respawnid", best_action_sim_respawn_first_pass.get_sim_id())
    ...
    def plan_action_continuous(self, other_ships_exist: bool, base_state_is_exact: bool, iterations_boost: bool = False, plan_stationary: bool = False) -> None:
        ...
        if self.game_state_to_base_planning['respawning']:
            ...
            # Simulate and look for a good move
            # print(f"Checking for imminent danger. We're currently at position {ship_state.x} {ship_state.y}")
            # print(f"Current ship location: {ship_state.x}, {ship_state.y}, ship heading: {ship_state.heading}")
            ...
            elif any((m.x, m.y) in self.game_state_to_base_planning['mine_positions_placed'] for m in self.game_state_to_base_planning['game_state'].mines):
    ...
    def actions(self, ship_state_dict: dict[str, Any], game_state_dict: dict[str, Any]) -> tuple[float, float, bool, bool]:
        ...
        #debug_print(f"\n\nTimestep {self.current_timestep}, ship id {ship_state.id} is at {ship_state.x} {ship_state.y}")
