    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool, bool]:
        ...
        while((f<30+self.current_frame) and not (self.targeted) and (closest_asteroid != {})):
            ...
            if (abs(angle_difference)<=6*(f-self.current_frame)):
                ...
                equalised_angle_diff = angle_difference/(f-self.current_frame) if f != self.current_frame else 999999  # Normalize the angle difference by the number of frames NOT PLUS ONE HERE DONT ASK ME WHY
